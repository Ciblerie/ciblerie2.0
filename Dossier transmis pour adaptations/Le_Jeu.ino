...
int score[MAX_SCORES] = {0};              
//String pseudo[MAX_SCORES] = {"J1", "J2", "J3", "J4", "J5"}; 
String tabScores[MAX_SCORES];
// Définition des joueurs
String pseudoJoueur[4] = {"J1", "J2", "J3", "J4"};
String pseudoJ1 = "J1"; // Valeur par défaut
//int scores[4] = {0, 0, 0, 0};  // Tableau des scores des joueurs
//int classement[4] = {0, 1, 2, 3};  // Indices des joueurs triés
char caracteres[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ,.!?'""/(){}[]<>^*@#&$~_-|`=+%";
int pseudoIndex = 0; // L'indice du caractère actuel du pseudo
String pseudo = "";   // Le pseudo en construction
#define MAX_PSEUDOS 50
String pseudos[MAX_PSEUDOS]; // Stockage des pseudos enregistrés
int totalPseudos = 0;
int indexAffichage = 0;  // Index pour navigation
int indexPrecedent = -1; // Stocke l'index précédent pour éviter l'affichage inutile
//bool modeSaisie = true; // True = saisie pseudo
bool modeSaisie = false; // False = affichage des pseudos
bool inCreationPseudo = false; //
bool disableCreationPseudo = false;  //
// Groupes de scores
...
int pointBonus[] = {0,0,0,0,0,0,0,0,0};
//int pointBonus5[] = {0,0,0,0,0,0,0,0,0};
int pointBonus5[9] = {0};  // Stocke le bonus pour chaque joueur
bool premiereTouche[9] = {true};  // Permet de savoir si c'est la première touche du joueur
unsigned long dernierTempsTouch[9] = {0};  // Stocke le temps de la dernière touche
int pointBonus10[] = {0,0,0,0,0,0,0,0,0};
int pointBonus15[] = {0,0,0,0,0,0,0,0,0};
int pointBonus25[] = {0,0,0,0,0,0,0,0,0};
int pointBonus50[] = {0,0,0,0,0,0,0,0,0};
int pointBonus100[] = {0,0,0,0,0,0,0,0,0};
int pointBonus150[] = {0,0,0,0,0,0,0,0,0};
int pointBonus200[] = {0,0,0,0,0,0,0,0,0};
int pointBonus250[] = {0,0,0,0,0,0,0,0,0};
...
void initialiserFichiersSD() {
  for (int i = 0; i < nombreGroupes; i++) {
    String fichierNom = String(groupes[i]) + ".csv";

    if (!SD.exists(fichierNom)) { // Vérifie si le fichier existe
      File fichier = SD.open(fichierNom, FILE_WRITE);
      if (fichier) {
        Serial.println("Création du fichier: " + fichierNom);
        fichier.close();
      } else {
...
void setup(){
  ...
  Serial1.begin(9600);  // Communication avec l'ESP32 WebSocket (pins 18 et 19)
  Serial2.begin(9600);  // Communication avec l'ESP32 Cloud (pins 16 et 17)
...
//  printFilenames();
  initialiserFichiersSD(); // Vérifie et crée les fichiers si besoin
...
//    CreationPseudo();
    ...
    if (!disableCreationPseudo) {
      CodeLedAleatoire();
      SonsAleatoires(); 
    }
  }
/* 
  if (inAffichageScores) {
    afficherScores();
  } else {
    if (!disableAffichageScores) {
      CodeLedAleatoire();
      SonsAleatoires(); 
    }
  }
*/
  if (inAffichageScores) {
    afficherScores();                // bloque tant qu’on est dans le menu des scores
    inAffichageScores = false;      // on quitte le mode score
    disableAffichageScores = false; // on réactive sons/LED
    MenuScreen0();                  // retour au menu
  } else {
    if (!disableAffichageScores) {
      CodeLedAleatoire();
      SonsAleatoires(); 
  ...
  // Ajoute ici ton code normal du menu et des boutons
  PresystemStart();
