...
void CSP1(){ //------------------------------------------------------------------------------------
  ...
  indexgroupe = 1;  // Toujours revenir au groupe SP avant le début de chaque partie
  AcquisitionCapteurs();
  if (initialisation==true){InitGameSP();}     
  if (partieEnCours==true){trtPartieEnCoursSP();} 
  if (partieFinie==true){trtPartieFinieSP();}
}
//---------------------------------------------FIN LOOP-------------------------------

void trtPartieFinieSP(){
    FinGame();   
