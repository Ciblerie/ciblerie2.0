...
void MS3(){ //------------------------------------------------------------------------------------
  ...
  AcquisitionCapteurs();
  if (initialisation==true){InitGameMS3();}     
  if (partieEnCours==true){trtPartieEnCoursMS3();} 
  if (partieFinie==true){trtPartieFinieMS3();}
}
//---------------------------------------------FIN LOOP-------------------------------

void trtPartieFinieMS3(){  
    FinGame();   
...
void GererInterruptionMS3()
{
  if ( joueurEnCours == nbJoueurs3 && tourEnCoursMS == nbToursMS && resteEnCoursMS == 1 ){
    testKiller();
    coursesCommencees = false;
    coursesFinies = false;
    partieEnCours = false;
  ...
  else if (  joueurEnCours == nbJoueurs3 && resteEnCoursMS == 1 )
  {
    myDFPlayer.stop();
    testKiller();;
    joueurEnCours = 1;            
