...
void LJ2(){ //---------------------------------------------FIN LOOP-------------------------------
  ...
  AcquisitionCapteurs();
  if (initialisation==true){InitGameLJ2();}   
  if (partieEnCours==true){trtPartieEnCoursLJ2();} 
  if (partieFinie==true){trtPartieFinieLJ2();}

}//---------------------------------------------FIN LOOP-------------------------------

void trtPartieFinieLJ2(){   
    FinGame();   
...
void GererInterruptionLJ2()
{
  if ( joueurEnCours == nbJoueurs2 && tourEnCoursLJ == nbToursLJ && resteEnCoursLJ == 1 ){
    testKiller();
    coursesCommencees = false;
    coursesFinies = false;
    partieEnCours = false;
  ...
  else if (  joueurEnCours == nbJoueurs2 && resteEnCoursLJ == 1 )
  {
    myDFPlayer.stop();
    testKiller();
    joueurEnCours = 1;            
